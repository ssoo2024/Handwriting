...
def draw_example(ch, src_font, dst_font, canvas_size, src_offset, dst_offset, filter_hashes):
    ...
    # src_img = cut_padding(src_img).resize((60,60))
    # src_img = add_padding(src_img, 34)
...
def get_font_offset(charset, font, canvas_size, filter_hashes):
    ...
    for c in sample:
        ...
        if not font_hash in filter_hashes:
            ...
            count += 1
    font_offset=np.true_divide(font_offset, count)
    return font_offset
...
def draw_handwriting(ch, src_font, canvas_size, src_offset, dst_folder):
    s = str(ch.encode('raw_unicode_escape')).replace("b\'\\\\u","").replace("\'","").upper()
...
args = easydict.EasyDict({ 
    "src_font":"./fonts/2.ttf",
    "dst_font":"./fonts/99.ttf",
    "filter":0,
    "shuffle":0,
    "char_size":80,
    "canvas_size":128,
